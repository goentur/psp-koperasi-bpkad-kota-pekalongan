...
export default function Index({ gate }: IndexGate) {
    ...
    const getData = async () => {
        setLoading(true)
        try {
            const response = await axios.post(route('tempat-saldo.data'), {
    ...
    const handleForm = (e: React.FormEvent) => {
        ...
        const routeName = isEdit ? route('tempat-saldo.update', data).toString() : route('tempat-saldo.store').toString()
        action(routeName, {
            preserveScroll: true,
            onSuccess: (e) => {setForm(false),reset(),alertApp(e),getData()},
            onError: (e) => {
    ...
    const handleHapus = (e: React.FormEvent) => {
        e.preventDefault()
        destroy(route('tempat-saldo.destroy', data), {
            preserveScroll: true,
