...
export default function Index() {
    const title = 'Persetujuan Angsuran'
    ...
    useEffect(() => {
        getData()
    }, [infoDataTabel.page, infoDataTabel.search, infoDataTabel.perPage, infoDataTabel.anggota])

    const getData = async () => {
        setLoading(true)
        try {
            const response = await axios.post(route('transaksi.dataTransaksiAngsuran'), {
    ...
    const handleHapus = (e: React.FormEvent) => {
        e.preventDefault()
        post(route('transaksi.terimaTransaksiAngsuran', {id:data.id}), {
            preserveScroll: true,
